...
def weighted_graph_bisection_problem_SA(L=None, R=None, random_numbers=None, initial_temperature=None,
                                        temperature_reducing_rate=None, k=None, transitions_each_phase=None):
    df = read_graph_from_csv().drop_duplicates(ignore_index=True)
    num_nodes = df[['node1', 'node2']].max().max() - df[['node1', 'node2']].min().min() + 1
    print(f"Graph:\n{df}")
    print(f"number of nodes: {num_nodes}\nnumber of edges: {len(df)}")
...
fast_calculate_cut_cost(L=np.asarray([3, 4, 6, 7, 10], dtype=np.int8), R=np.asarray([2, 5, 1, 8, 9], dtype=np.int8))